...
export class APIBookService {
  ...
  searchBooks(bookSearchParam: string, startIndex: number = 0) {
    this.http.get<any>(`https://www.googleapis.com/books/v1/volumes?q=${bookSearchParam}+intitle:${bookSearchParam}&startIndex=${startIndex}&key=AIzaSyDxqCMvt7YcqbT4m-9J2ONJmlK35yxsCkE`)
      .subscribe(data => {
